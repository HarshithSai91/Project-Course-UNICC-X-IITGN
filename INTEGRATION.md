...
1. **Keyword-Only Baseline:** Uses a deterministic lexical proxy (unigram Jaccard overlap). This is NOT actual BM25 — it is a lexical evaluation proxy only.
2. **Vector-Only Baseline:** Uses a deterministic semantic proxy (character 3-gram overlap / cosine). This is NOT actual BGE-M3 dense retrieval — it is a vector similarity proxy only.
3. **Weighted Hybrid Baseline:** Combines lexical and vector proxies. This is NOT actual RRF or Suhani's hybrid retrieval — it is a weighted evaluation proxy only.
